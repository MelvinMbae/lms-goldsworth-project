...
class FetchImage(Resource):
    def get(self):
        ...
        if 'lecturer' in user.email:
            photo = send_from_directory(
                app.config['UPLOAD_PATH'], user.teacher.image_url)
            return photo
        elif 'student' in user.email:
            photo = send_from_directory(
                app.config['UPLOAD_PATH'], user.student.image_url)
            return photo
...
class StudentView(ModelView):
    form_columns = ['firstname', 'lastname', 'password',
                    'email', 'personal_email', 'parent', 'image_url']


class TeacherView(ModelView):
    form_columns = ['firstname', 'lastname', 'password', 'email',
                    'personal_email', 'expertise', 'image_url', 'department']


class ParentView(ModelView):
    form_columns = ['firstname', 'lastname',
                    'password', 'email', 'child', 'image_url']
...
class Assignments(Resource):
    ...
    def post(self):
        ...
        new_assignment = Assignment(
            ...
            due_date=datetime.strptime(assignment_data['due_date'],"%Y-%m-%dT%H:%M").date(),
...
class AssignmentbyId(Resource):
    ...
    def patch(self, id):
        ...
        for attr in assignment_data:
            if attr in ['due_date']:
                setattr(assignment, attr, datetime.strptime(
                    assignment_data[attr], "%Y-%m-%dT%H:%M").date())
            else:
